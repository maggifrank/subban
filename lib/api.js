...
import { normalize, addTrip, removeLastTrip, removeTripAt, setTripPool, clearTrips, updateSettings } from './state.js';
...
export async function handle({ method, path, body, token }, deps, expectedToken) {
  ...
  if (path === '/api/trips' && method === 'POST') {
    ...
    if (pool !== undefined) {
      if (typeof pool !== 'object' || pool === null ||
          ...
          typeof pool.name !== 'string' || !pool.name.trim()) {
        return json(400, { error: 'Invalid pool' });
      }
    }
    return json(200, await mutate(store, (s) => addTrip(s, at, pool)));
  }
  ...
  /* Set or clear the pool on an existing trip. `pool: null` detaches it. */
  if (path === '/api/trips/one/pool' && method === 'PUT') {
    if (typeof body?.at !== 'string') return json(400, { error: 'Expected { at }' });
    ...
    return json(200, await mutate(store, (s) => setTripPool(s, body.at, pool)));
  }
