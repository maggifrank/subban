...
/* The annual card covers three pools; everything else is logged for the record
   ...
   keep counting exactly as they did before. */
export function countsForCard(trip, pools) {
  if (!trip.pool) return true;
  const pool = pools.find((p) => p.id === trip.pool);
  return pool ? pool.card === true : true;
}

export const cardTrips = (state) => state.trips.filter((t) => countsForCard(t, state.pools));
...
export function poolCounts(state) {
  ...
  const cards = new Map(state.pools.map((p) => [p.id, p.card === true]));
  const rows = [...counts].map(([id, count]) => ({
    id, count,
    name: id === null ? null : (names.get(id) ?? id),
    card: id === null ? true : (cards.get(id) ?? true)
  }));
