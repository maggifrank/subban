 ...
 * among them — are absent from the listing and were taken from there.
 * Seljavallalaug came from neither: the site files it under heitar_laugar, a
 * separate section of natural pools with a sitemap of its own that the pool
 * directory does not cover. Positions were then looked up in OpenStreetMap by
 * name: 62 of the 108 have one, so those can be detected by location. The rest
 * carry a name and no position — they cannot be auto-detected, but they appear
 * in the picker, so a swim can still be attributed to them from the History
 * list.
 *
