 ...
 *
 * Positions were then looked up in OpenStreetMap by name: 81 of the 131 have
 * one, so those can be detected by location. The rest carry a name and no
 * position — they cannot be auto-detected, but they appear in the picker, so a
 * swim can still be attributed to them from the History list. The five without
 * one here are all wild pools — Hveragil, Kerið á Húsavíkurhöfða, Lægðin, Siká
 * and Laugarnes – Birkimel — which OSM does not name.
 *
